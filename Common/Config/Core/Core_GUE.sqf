...
_i = _i + [['','',300,0,1,0,'Defense',0,'TK GUE']];
...
for '_z' from 0 to (count _c)-1 do {
	...
	if (isNil '_get') then {
		...
		diag_log Format ["[WFBE (INIT)] Core_GUE: Duplicated Element found '%1'",(_c select _z)];
	};
};

diag_log Format ["[WFBE (INIT)] Core_GUE: Initialization (%1 Elements) - [Done]",count _c];