...
_u = _u + ['NLAW'];
_i = _i + [['','','CfgMagazines','secondary',175,0,false,false,0,0]];

/* Primary Weapons - West */
_u = _u + ['BAF_AS50_scoped'];
_i = _i + [['','','CfgWeapons','',340,2,true,false,0,0]];

_u = _u + ['BAF_AS50_TWS'];
_i = _i + [['','','CfgWeapons','',420,3,false,false,0,0]];

_u = _u + ['BAF_LRR_scoped'];
_i = _i + [['','','CfgWeapons','',310,2,true,false,0,0]];
...
_u = _u + ['BAF_L7A2_GPMG'];
_i = _i + [['','','CfgWeapons','',170,0,false,false,0,0]];
...
_i = _i + [['','','CfgVehicles','',95,2,true,false,0,0]];

_u = _u + ['BAF_L2A1_ACOG_minitripod_bag'];
_i = _i + [['','','CfgVehicles','',165,1,false,false,0,0]];

_u = _u + ['BAF_L2A1_ACOG_tripod_bag'];
_i = _i + [['','','CfgVehicles','',175,1,false,false,0,0]];

_u = _u + ['BAF_GPMG_Minitripod_D_bag'];
_i = _i + [['','','CfgVehicles','',175,0,false,false,0,0]];
...
/* Misc - West */
_u = _u + ['BAF_L109A1_HE'];
_i = _i + [['','','CfgMagazines','CfgMagazines',12,0,true,false,0,0]];
	...
	if (isClass (configFile >> _config >> _element)) then {
		...
		if (isNil '_get') then {
			...
			if ((_get select 2) == _config) then {
				diag_log Format ["[WFBE (INIT)] Core_BAF_G: Duplicated Loadout Element found '%1' for class '%2'",_element,_config];
			} else {
		...
		diag_log Format ["[WFBE (ERROR)] Core_BAF_G: Loadout Element '%1' with class '%2' is not a defined a class!",_element,_config];
	};
};

diag_log Format ["[WFBE (INIT)] Core_BAF_G: Loadout Initialization (%1 Elements) - [Done]",count _u];