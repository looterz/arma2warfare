    ...
    public partial class ILBlock
    {
        ...
        public class InlineLogic
        {
            ...
            public void Resolve(ILFlowStackItem s, ILIfElseConstruct iif)
            {
                ...
                if (ResolveIf(iif, OpCodes.Brfalse_S, false, true, false, true)) return;
                if (ResolveIf(iif, OpCodes.Brtrue_S, false, true, true, false)) return;
            ...
            void ResolveIfSub(ILIfElseConstruct iif)
            {
                ...
                {
                    if (ResolveIfSubExt(iif, i, OpCodes.Brfalse_S, false, true, false, true)) continue;
                    if (ResolveIfSubExt(iif, i, OpCodes.Brtrue_S, false, true, true, false)) continue;

                    // we need additional testing 

                    Task.Error("bad opIf in external conditions");

                    Debugger.Break();
                }
            }

            bool ResolveIfSubExt(ILIfElseConstruct owner,
                ILIfElseConstruct opIf,
                OpCode op,
                bool orValue,
                bool andValue,
                bool negLhs,
                bool negRhs)
            {
                if (opIf.Branch == op)
                {
                    ...
                    x.lhs = new InlineLogic(ILBlockInlineLogicSpecialType.Value, opIf.Branch.StackBeforeStrict[0]);
                    x.rhs = lhs;



                    if (owner.IsTResult(orValue))
                    {
                        x.hint = ILBlockInlineLogicSpecialType.OrOperator;
                        x.lhs.IsNegative = negLhs && (owner.Branch == opIf.Branch.OpCode);
                        x.rhs.IsNegative = negRhs && (owner.Branch == opIf.Branch.OpCode);

                    }
                    if (owner.IsTResult(andValue))
                    {
                    ...
                    if ((owner.IsTResult(false) && opIf.Branch == OpCodes.Brfalse_S && owner.Branch == OpCodes.Brtrue_S)
                       || (owner.IsTResult(false) && opIf.Branch == OpCodes.Brtrue_S && owner.Branch == OpCodes.Brtrue_S)
                       || (owner.IsTResult(false) && opIf.Branch == OpCodes.Brfalse_S && owner.Branch == OpCodes.Brfalse_S)
                       || (owner.IsTResult(true) && opIf.Branch == OpCodes.Brtrue_S && owner.Branch == OpCodes.Brfalse_S))
                    {
            ...
            bool ResolveIf(ILIfElseConstruct iif, OpCode o, bool and_value, bool or_value, bool lhs_and_neg, bool lhs_or_neg)
            {
                if (iif.Branch == o)
                {
                    InlineLogic x = new InlineLogic();
            ...
            public void ValidateInlineAssigment()
            {
                ...
                if (loop != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception("unknown while condition at " + this.Instruction.OwnerMethod.ToString()
