...
namespace jsc
{
    ...
    {
        readonly RecursionGuard _inlineIfElseConstructGuard = new RecursionGuard();

        Variant<ILIfElseConstruct> _cachedIIF;
        ...
        public ILIfElseConstruct InlineIfElseConstruct
        {
            ...
            {
                if (_cachedIIF.Dirty)
                    return _cachedIIF.Value;

                _cachedIIF.Dirty = true;

                using (_inlineIfElseConstructGuard.Lock)
                {
        ...
        private ILIfElseConstruct InternalGetIfElseConstructVerified()
        {
            ...
            if (t.Prev.Flow != Flow)
            {
                ...
                {
                    throw new Exception("unable to recover from if/else detection at " + OwnerMethod.DeclaringType.FullName + "." + OwnerMethod.Name);
                }
            }
