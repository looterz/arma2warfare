...
paramRealisticBallistic = false
paramTownDefenceConstruction = true;
...
if !(isNil "paramsArray") then {
	...
	if ((paramsArray select _u) == 0) then {paramRealisticBallistic = false} else {paramRealisticBallistic = true};_u = _u + 1; //--- Town Defence Construction Menu	
	if ((paramsArray select _u) == 0) then {paramTownDefenceConstruction = false} else {paramTownDefenceConstruction = true};_u = _u + 1; //--- Town Defence Construction Menu	
};
...
//--- Repair wheel
wheel_procInitComm = Compile PreprocessFile "addon\wheel\common\procInitComm.sqf";
if (local player) then {ExecVM "addon\Init_Client.sqf"};