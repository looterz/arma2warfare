...
if ((side _killer != _side)&&(isPlayer(_killer)))  then {
	_bounty = if ((vehicle _killer) isKindOf 'Air') then { 2000 } else { 10500 };	
	...
	WFBE_LocalizeMessage = [_killerVehName,'CLTFNCLOCALIZEMESSAGE',['HeadHunterReceiveBounty',_bounty,_killedName,_killerId]];
	publicVariable 'WFBE_LocalizeMessage';
	if (isHostedServer) then {[_killerVehName,'CLTFNCLOCALIZEMESSAGE',['HeadHunterReceiveBounty',_bounty,_killedName,_killerId]] Spawn HandlePVF};
	};
