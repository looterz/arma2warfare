...
if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_PLAYERS_ATTACK_AIR',[_vehicle,_uniqueLabel,_uniqueIndex]] Spawn HandlePVF};
_jipHandler = [_vehicle,_uniqueLabel,_uniqueIndex] Spawn {
	...
	while {true} do {
		...
		if (_update) then {
			...
			publicVariable 'WFBE_M_PLAYERS_Attack_Air';
			if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_PLAYERS_ATTACK_AIR',[_chopper,_uniqueLabel,_uniqueIndex]] Spawn HandlePVF};
		};
	};
};
