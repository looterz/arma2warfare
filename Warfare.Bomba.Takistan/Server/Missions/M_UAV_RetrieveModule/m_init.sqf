...
if (paramBoundaries) then {
	...
	_boundariesxy = ('WFBE_BOUNDARIESXY' Call GetNamespace) - (_markerSize*2);
	...
	while {_attempts != -1} do {
		...
		if (count (_randomPos isFlatEmpty [20/8,0,0.6,20,0,false,objNull]) > 0) then {
			...
			if (count(_randomPos nearEntities [['Man','Car','Motorcycle','Ship','Air','Tank','StaticWeapon'],_safeRadius]) == 0) exitWith {_attempts = -1};
...
if (random 100 > 35) then {
	...
	//--- Create the team.
	_returned = [_list,_defensePos,resistance,paramResVehLock] Call CreateTeamTemplate;
...
_jipHandler = [_markerSize,_uniqueLabel,_uniqueIndex,_uav] Spawn {
	while {true} do {
		...
		if (_update) then {
			...
			publicVariable 'WFBE_M_UAV_RetrieveModule';
			if (!isMultiplayer || (isServer && local player) || local player) then {[nil,'CLTFNCM_UAV_RETRIEVEMODULE',_this] Spawn HandlePVF};
		};
	};
};
...
if (({alive _x} count _units) > 0) then {
	//--- Clean up.
	[_units, [[(_randomPos select 0)+random(_markerSize)-random(_markerSize),(_randomPos select 1)+random(_markerSize)-random(_markerSize)], 250] Call GetSafePlace] Spawn {
		Private ['_grp','_pos','_team','_units','_vehicles'];
		_units = _this select 0;
		_vehicles = _this select 1;
		...
		//--- Remove vehicles.
		{ if (!isPlayer (driver _x)) then { deleteVehicle _x} } forEach _vehicles;
