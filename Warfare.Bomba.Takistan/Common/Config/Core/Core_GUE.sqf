...
/* Light Vehicles */
_c = _c + ['TT650_Gue'];
_i = _i + [['','',150,15,1,0,1,0,'Guerilla']];

_c = _c + ['V3S_Gue'];
_i = _i + [['','',175,15,1,0,1,0,'Guerilla']];
...
/* Heavy Vehicles */
_c = _c + ['BMP2_Gue'];
_i = _i + [['','',2200,30,3,1,2,0,'Guerilla']];

_c = _c + ['T72_Gue'];
_i = _i + [['','',3200,35,3,2,2,0,'Guerilla']];
...
for '_z' from 0 to (count _c)-1 do {
	if (isClass (configFile >> 'CfgVehicles' >> (_c select _z))) then {
		...
		format["Core_GUE: Duplicated Element found '%1'",(_c select _z)] call LogHigh;
		};
	} else {
		format["Core_GUE: Element '%1' is not a valid class.",(_c select _z)] call LogHigh;
	};
};

Format ["Core_GUE: Initialization (%1 Elements) - [Done]",count _c] call LogMedium;