...
_i = _i + [['','',175,15,1,0,1,0,'Takistani Guerilla']];
...
for '_z' from 0 to (count _c)-1 do {
	if (isClass (configFile >> 'CfgVehicles' >> (_c select _z))) then {
		...
		format["Core_TKGUE: Duplicated Element found '%1'",(_c select _z)] call LogHigh;
		};
	} else {
		format["Core_TKGUE: Element '%1' is not a valid class.",(_c select _z)] call LogHigh;
	};
};

Format ["Core_TKGUE: Initialization (%1 Elements) - [Done]",count _c] call LogMedium;