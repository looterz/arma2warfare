...
/* Air Vehicles */
_c = _c + ['Ka137_PMC'];
_i = _i + [['','',3000,35,1,0,3,0,'PMC']];

_c = _c + ['Ka137_MG_PMC'];
_i = _i + [['','',3500,35,2,0,3,0,'PMC']];

_c = _c + ['Ka60_PMC'];
_i = _i + [['','',6000,43,1,0,3,0,'PMC']];
...
for '_z' from 0 to (count _c)-1 do {
	if (isClass (configFile >> 'CfgVehicles' >> (_c select _z))) then {
		...
		format["Core_PMC: Duplicated Element found '%1'",(_c select _z)] call LogHigh;
		};
	} else {
		format["Core_PMC: Element '%1' is not a valid class.",(_c select _z)] call LogHigh;
	};
};
