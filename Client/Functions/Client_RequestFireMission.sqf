...
_type = artilleryNames Find (typeOf (_units select 0));
if (_type < 0) exitWith {_type};
_reloadTime = artilleryReloadTimes select _type;
_burst = artilleryBurst select _type;
...
for [{_count1 = 0},{_count1 < _burst},{_count1 = _count1 + 1}] do {
	...
	for [{_count = Count _units - 1},{_count >= 0},{_count = _count - 1}] do {
		[_units Select _count,_destination,Side player,artyRange] Spawn FireArtillery;
	};
