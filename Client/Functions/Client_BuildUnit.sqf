Private ["_factoryType", "_building","_built","_cpt","_commander","_crew","_description","_direction","_distance","_driver","_factoryType","_group","_gunner","_index","_isMan","_locked","_longest","_position","_queu","_queu2","_ret","_soldier","_waitTime","_type","_unique","_unit","_unitIndex","_vehi","_vehicle","_vehicles"];
...
if (_index != -1) then {
	...
	_unitIndex = (Format ["WFBE_%1%2UNITS",sideJoinedText,_factoryType] Call GetNamespace) find _unit;
	_waitTime = (Format ["WFBE_%1%2TIMES",sideJoinedText,_factoryType] Call GetNamespace) select _unitIndex;
	_description = (Format ["WFBE_%1%2DESCRIPTIONS",sideJoinedText,_factoryType] Call GetNamespace) select _unitIndex;
...
if (_isMan) then {
	...
	if (_unit == (Format['WFBE_%1REPAIRTRUCK',sideJoinedText] Call GetNamespace)) then {
		if (('WFBE_VICTORYCONDITION' Call GetNamespace) != 1) then {
			_vehicle setVehicleInit Format ["this addAction [localize 'STR_WF_BuildMenu_Repair','Client\Action\Action_BuildRepair.sqf', [], 99, false, true, '', '!townConstructionInRange && side player == side _target && alive _target && player distance _target <= %1'];this addAction [localize 'STR_WF_Repair_MHQ','Client\Action\Action_RepairMHQ.sqf', [], 98, false, true, '', 'alive _target']",'WFBE_REPAIRTRUCKRANGE' Call GetNamespace];
		} else {
			_vehicle setVehicleInit Format ["this addAction [localize 'STR_WF_BuildMenu_Repair','Client\Action\Action_BuildRepair.sqf', [], 99, false, true, '', '!townConstructionInRange && side player == side _target && alive _target && player distance _target <= %1']",'WFBE_REPAIRTRUCKRANGE' Call GetNamespace];
		};
		processInitCommands;
	};
	if (_unit == (Format['WFBE_%1SUPPLYTRUCK',sideJoinedText] Call GetNamespace)) then {[_vehicle,sideJoined] ExecFSM "Client\FSM\updatesupply.fsm"};
	...
	if (_unit isKindOf "Air") then {
		...
		if (AARadar) then {_init = _init + Format["nullReturn = [this,%1] ExecVM 'Common\Common_AARadarMarkerUpdate.sqf';",sideJoined]};
	...
	if (_unit in ('WFBE_BALANCEDUNITS' Call GetNamespace) && balancing) then {[_vehicle] Spawn BalanceInit};
	//--- Vehicles Init End.
