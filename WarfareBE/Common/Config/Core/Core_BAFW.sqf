...
_i = _i + [['','',965,20,3,1,1,0,'British (Woodland)']];

_c = _c + ['BAF_Jackal2_L2A1_W'];
_i = _i + [['','',725,21,3,0,1,0,'British (Woodland)']];

/* Heavy Vehicles */
_c = _c + ['BAF_FV510_W'];
_i = _i + [['','',6250,30,2,2,2,0,'British (Woodland)']];
...
for '_z' from 0 to (count _c)-1 do {
	...
	if (isNil '_get') then {
		...
		diag_log Format ["[WFBE (INIT)] Core_BAFW: Duplicated Element found '%1'",(_c select _z)];
	};
};

diag_log Format ["[WFBE (INIT)] Core_BAFW: Initialization (%1 Elements) - [Done]",count _c];