...
for '_z' from 0 to (count _c)-1 do {
	...
	if (isNil '_get') then {
		...
		diag_log Format ["[WFBE (INIT)] Core_TKGUE: Duplicated Element found '%1'",(_c select _z)];
	};
};

diag_log Format ["[WFBE (INIT)] Core_TKGUE: Initialization (%1 Elements) - [Done]",count _c];