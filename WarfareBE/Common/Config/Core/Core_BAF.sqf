...
_i = _i + [['','',9500,45,1,0,3,0,'British']];

_c = _c + ['CH_47F_BAF'];
_i = _i + [['','',10500,40,2,0,3,0,'British']];

_c = _c + ['BAF_Apache_AH1_D'];
_i = _i + [['','',39500,60,2,2,3,0,'British']];

_c = _c + ['AW159_Lynx_BAF'];
_i = _i + [['','',23500,35,2,1,3,0,'British']];
...
for '_z' from 0 to (count _c)-1 do {
	...
	if (isNil '_get') then {
		...
		diag_log Format ["[WFBE (INIT)] Core_BAF: Duplicated Element found '%1'",(_c select _z)];
	};
};

diag_log Format ["[WFBE (INIT)] Core_BAF: Initialization (%1 Elements) - [Done]",count _c];