	...
	if (isClass (configFile >> _config >> _element)) then {
		...
		if (isNil '_get') then {
			...
			if ((_get select 2) == _config) then {
				diag_log Format ["[WFBE (INIT)] Core_PMC_G: Duplicated Loadout Element found '%1' for class '%2'",_element,_config];
			} else {
		...
		diag_log Format ["[WFBE (ERROR)] Core_PMC_G: Loadout Element '%1' with class '%2' is not a defined a class!",_element,_config];
	};
};

diag_log Format ["[WFBE (INIT)] Core_PMC_G: Loadout Initialization (%1 Elements) - [Done]",count _u];