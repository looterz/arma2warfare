...
if (!paramSpacebar || !paramTabLock || !paramTacView) then {
	...
	if (!paramTabLock) then {
		if (_condition != "") then {_condition = _condition + "+"};
		_condition = _condition + "(actionKeys 'VehLockTargets')+(actionKeys 'LockTargets')";
	};
	if (!paramTacView) then {
		if (_condition != "") then {_condition = _condition + "+"};
		_condition = _condition + "(actionKeys 'TacticalView')";
	};
