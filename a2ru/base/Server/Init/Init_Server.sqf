...
if (paramAI) then {
	...
	_i = 1;
	{
		if !(isNil "_x") then {
			...
			if (_addAiVehicleStart) then {
				_group = _x;
				{_group addVehicle _x} forEach _vehEast;
			};
	...
	_i = 1;
	{
		if !(isNil "_x") then {
			...
			if (_addAiVehicleStart) then {
				_group = _x;
				{_group addVehicle _x} forEach _vehWest;
			};
