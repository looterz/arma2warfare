...
*/
Private ['_attempts','_funds','_get','_leader','_name','_sideJoined','_sideOrigin','_slotIndex','_team','_uid','_units'];
_uid = _this select 0;
...
if (_leader isKindOf westSoldierBaseClass) then {_sideJoined = west};
if (_leader isKindOf eastSoldierBaseClass) then {_sideJoined = east};
...
if !(paramKeepAI) then {
	diag_log Format ["[WFBE (INFORMATION)][frameno:%7 | ticktime:%8] Server_PlayerConnected: Removing units (Team: %1, Team Side: %2, Team Leader: %3) for Player %4 (%5)   DEBUG:%6.",_team,side _team,name _leader,_name,_uid,_get,diag_frameno,diag_tickTime];
	_units = units _team;
...
//--- ISIS.
if (('WFBE_ISIS' Call GetNamespace) != 0) then {_leader removeAllEventHandlers "handleDamage"};
...
if (isNil '_get') exitWith {
	...
	*/
	[Format["WFBE_JIP_USER%1",_uid],[_uid,_slotIndex,0,_sideJoined,_sideJoined,if (mysql) then {round(time)} else {0},vehicleVarName _leader],true] Call SetNamespace;

	Call Compile Format ["%1Funds%2 = %3; publicVariable '%1Funds%2';",str _sideJoined,_slotIndex+1,Format ["WFBE_%1STARTINGMONEY",str _sideJoined] Call GetNamespace];
};
...
_get set [6,vehicleVarName _leader];
