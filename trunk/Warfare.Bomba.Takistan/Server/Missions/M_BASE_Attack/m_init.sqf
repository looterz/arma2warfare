...
//--- Grab the HQ.
_isHQDeployed = (_runForce) Call GetSideHQDeployed;
...
if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_BASE_ATTACK',[_size/2,_uniqueLabel,_uniqueIndex,_target,_groups]] Spawn HandlePVF};
_jipHandler = [_runFor,_size,_uniqueLabel,_uniqueIndex,_target,_groups] Spawn {
	Private ['_groups','_runFor','_size','_target','_uniqueLabel','_uniqueIndex'];
	...
	while {true} do {
		...
		if (_update) then {
			...
			publicVariable 'WFBE_M_BASE_Attack';
			if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_BASE_ATTACK',[_size,_uniqueLabel,_uniqueIndex,_target,_groups]] Spawn HandlePVF};
		};
	};
};
...
while {_i < _grps} do {
	...
	_retVal = [_teamType,_pos,resistance,paramBoundaries,_group] Call CreateTeamTemplate;
