...
_towns = [];
{
	if (_x getVariable 'sideID' == WESTID) then {_towns = _towns + [_x]};
} forEach towns;
...
_grps = 1;
if (_sv < 30) then {_grps = 2};
if (_sv >= 30 && _sv < 50) then {_grps = 3};
if (_sv >= 50 && _sv < 80) then {_grps = 4};
if (_sv >= 80 && _sv < 100) then {_grps = 5};
if (_sv >= 100 && _sv < 120) then {_grps = 6};
if (_sv >= 120) then {_grps = 7};
...
if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_TOWN_ATTACK',[_size/2,_uniqueLabel,_uniqueIndex,_town,_groups]] Spawn HandlePVF};
_jipHandler = [_runFor,_size,_uniqueLabel,_uniqueIndex,_town,_groups] Spawn {
	Private ['_groups','_runFor','_size','_town','_uniqueLabel','_uniqueIndex'];
	...
	while {true} do {
		...
		if (_update) then {
			...
			publicVariable 'WFBE_M_TOWN_Attack';
			if (!isMultiplayer || (isServer && local player) || local player) then {[_runFor,'CLTFNCM_TOWN_ATTACK',[_size,_uniqueLabel,_uniqueIndex,_town,_groups]] Spawn HandlePVF};
		};
	};
};
