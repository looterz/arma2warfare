...
_i = _i + [['','',450,25,2,0,1,0,'Takistani']];

_c = _c + ['BTR40_MG_TK_INS_EP1'];
_i = _i + [['','',600,25,2,0,1,0,'Takistani']];
...
_i = _i + [['','',425,17,1,1,1,0,'Takistani']];
...
_c = _c + ['T34_TK_EP1'];
_i = _i + [['','',2900,25,3,1,2,0,'Takistani']];
...
for '_z' from 0 to (count _c)-1 do {
	if (isClass (configFile >> 'CfgVehicles' >> (_c select _z))) then {
		...
		format["Core_TK: Duplicated Element found '%1'",(_c select _z)] call LogHigh;
		};
	} else {
		format["Core_TK: Element '%1' is not a valid class.",(_c select _z)] call LogHigh;
	};
};

Format ["Core_TK: Initialization (%1 Elements) - [Done]",count _c] call LogMedium;